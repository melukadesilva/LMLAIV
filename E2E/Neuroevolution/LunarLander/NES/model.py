...
class PolicyModel:
    def __init__(self, layer_shapes: list) -> None:
        ...
        self.model_weights = tuple(np.zeros((layer_shapes[i - 1], layer_shapes[i])) for i in range(
                                                                                            1, len(layer_shapes)))

        self.model_biases = tuple(np.zeros((layer_shapes[i])) for i in range(1, len(layer_shapes)))

        self.weight_shapes = [np.prod(w.shape) for w in self.model_weights]
        self.bias_shapes = [b.shape[0] for b in self.model_biases]

        # print(self.bias_shapes)
        def sum_e(x, y): return x + y

        self.num_weights = reduce(sum_e, self.weight_shapes)
        self.num_biases = reduce(sum_e, self.bias_shapes)
    ...
    def build(self) -> np.ndarray:
        x = np.random.uniform(-1, 1, size=(1, self.layer_shapes[0]))
        out = self(x)
        print(out.shape)
        return out
